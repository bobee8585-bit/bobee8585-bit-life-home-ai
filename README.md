...
`MAP_ROUTE_PROVIDER=GOOGLE_ROUTES`와 서버 전용 API 키가 설정되면 한 번의
경로 행렬 요청으로 모든 후보 구간을 조회하고, 최대 5개 매물의 전체 순열 중
교통 반영 예상 이동시간이 가장 짧은 순서를 선택합니다. 자동차 경로는
`TRAFFIC_AWARE_OPTIMAL`을 사용하며 응답에는 공급자, 조회 시각, 구간별
교통 반영 여부, 평시 대비 지연시간과 최적화 기준을 함께 표시합니다.

지도 공급자가 비활성화됐거나 일시적으로 응답하지 않으면 기존 Haversine
직선거리·도로 보정계수·교통수단별 평균속도 방식으로 자동 대체합니다. 이때
`fallbackUsed`와 사유를 명시해 공급자 기반 결과로 오인하지 않게 합니다.
지도 API 키는 서버 환경변수에만 두며 앱 설정이나 동선 응답에 노출하지
않습니다. 실제 예약은 제안된 시간마다 별도로 요청하고 각 매물 등록자의
승인을 받아야 확정됩니다.
